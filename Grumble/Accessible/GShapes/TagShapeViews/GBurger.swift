...
public struct GBurger: View {
    private static var imgPath: String = "burger_"
    private var boundingSize: CGSize
    private var idleData: CGFloat
    private var tossData: CGFloat
    ...
    private func imageAsset(path: String, scale: CGFloat, x: CGFloat = 0, y: CGFloat = 0) -> some View {
        return Image(GBurger.imgPath + path)
            .resizable()
            .aspectRatio(contentMode: .fit)
            .frame(width: bWidth() * scale)
            .offset(x: self.bWidth() * x, y: self.bHeight() * y)
    }
    
    private var topBun: some View {
        self.imageAsset(path: "TopBun", scale: 0.95 + 0.04 * self.idleData, y: 0.04 - 0.04 * self.idleData)
    }
    ...
    private var lettuce: some View {
        self.imageAsset(path: "Lettuce", scale: 0.9 + 0.03 * self.idleData, x: 0.02 * self.idleData, y: 0.28 - 0.02 * self.idleData)
    }
    
    private var tomato: some View {
        self.imageAsset(path: "Tomato", scale: 0.8 + 0.02 * self.idleData, y: 0.4 - 0.02 * self.idleData)
    }
    ...
    private var patty: some View {
        self.imageAsset(path: "Patty", scale: 0.9 - 0.03 * self.idleData, y: 0.48 + 0.02 * self.idleData)
    }
    
    private var bottomBun: some View {
        self.imageAsset(path: "BottomBun", scale: 0.85 - 0.05 * self.idleData, y: 0.58 + 0.02 * self.idleData)
    }
